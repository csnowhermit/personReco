...
from search import getColorArr, get_zhcn_number

'''
    多线程处理视频：解决消费端跟不上读取端速度的问题
'''
...
def read_video(opt,
    ...
    for i, (path, img, im0, vid_cap) in enumerate(dataloader):
        ...
        lock.release()
    with open("testData/frame_buffer.txt", encoding="utf-8") as fo:
        fo.writelines(frame_buffer)
...
def detect(opt,
    ...
    os.makedirs(str(output))  # make new output folder
    ...
    while True:
        if frame_buffer.size() > 0:
            lock.acquire()
            path, img, im0, vid_cap = frame_buffer.pop()
            print("path:", path)
            lock.release()
            ...
            #
            #         ################### 开始统计总人数 ###################
            #         if 'persons' not in personNumsDict.keys():
            #             personNumsDict['persons'] = n  # 总人数n
            #         else:
            #             personNumsDict['persons'] = personNumsDict['persons'] + n
            #         ################### 统计总人数结束 ###################
            #
            ...
            #                         personNumsDict[str(k)] = 1
            #                     else:
            #                         personNumsDict[str(k)] = personNumsDict[str(k)] + 1
            #                     ################### 统计该类型人员的人数 结束 ###################
            #
            ...
            #                     # plot_one_box(gallery_loc[index], im0, label='%s:%s' % (pidNameInfo[str(k)][0], tmp_mat[index]), color=getColorArr(pidNameInfo[str(k)][1]))
            #
            ...
            #                     color = getColorArr(pidNameInfo[str(k)][1])  # 画框颜色
            #
            ...
            #                         # cv2.waitKey()
            # ##############################
            # cv2.imshow("person-search-multi", im0)
            # cv2.waitKey(1)
            cv2.imwrite("testData/out/%4d.jpeg" % num, im0)
            num = num + 1
...
if __name__ == '__main__':
    ...
    t_read = threading.Thread(target=read_video, args=(opt,
                                                       frame_buffer,
                                                       lock,
                                                       opt.images,
                                                       416))
    ...
    # t_detect = threading.Thread(target=detect, args=(opt.cfg,
    #                                                  opt.data,
    #                                                  opt.weights,
    #                                                  frame_buffer,
    #                                                  lock,
