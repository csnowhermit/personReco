...
class  ICanvas(tk.Frame):
    ...
    def add(self, img, wigth, height):
        # print("add-img:", type(img))
        img = Image.fromarray(cv2.cvtColor(img, cv2.COLOR_BGR2RGB))    # cv2转为PIL，BGR-->RGB
        img = img.resize(size=(wigth, height))    # resize到画布大小
        # print("转换后：", type(img))
        if isinstance(img, np.ndarray):
            img = Image.fromarray(img.astype(np.uint8))
        self.img = img
        self.img_w, self.img_h = img.size    # 宽，高
        if self.first_add:
    ...
    def draw(self, x, y):
        ...
        # print("self.img:", type(self.img))    # <class 'PIL.Image.Image'>
        img = self.img.resize((int(self.img_w * self.scale), int(self.img_h * self.scale)))
        # print("widgets-img:", type(img))    # <class 'PIL.Image.Image'>
